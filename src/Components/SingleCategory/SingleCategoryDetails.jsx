...
import "./SingleCategoryDetails.css";
...
const SingleCategoryDetails = () => {
  ...
  const fetchCategoryProducts = async () => {
    try {
      ...
      let menProducts = data?.data?.filter((item) => item.gender === "Men");
      ...
      setBrands(
        Array.from(
          ...
          new Set( // to store unique elements in the array.
            data.data
              .filter((item) => item.gender === "Men" && item.brand)
              .map((item) => item.brand)
          )
        )
      );
      ...
      setCategoryProducts(menProducts.length ? menProducts : data.data);
    } catch (error) {
      console.log(error);
    }
  };

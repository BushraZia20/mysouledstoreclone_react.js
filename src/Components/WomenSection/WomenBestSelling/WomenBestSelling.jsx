...
const WomenBestSelling = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const womenProducts = data?.data?.filter(
        (item) => item.gender === "Women" && item.sellerTag === "best seller"
      );
  ...
  return (
    <div>
      ...
      {currentIndex !== 0 && (
        <button
          className="women_bestSelling_btn women_bestSelling_prev_btn"
      ...
      <button
        className="women_bestSelling_btn women_bestSelling_next_btn"
        onClick={nextSlides}
      >
        <FaAngleRight
          style={{
